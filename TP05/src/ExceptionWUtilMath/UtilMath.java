...
public class UtilMath {
	...
	/**
	 * Calcule la factorielle d'un nombre entier avec un algorithme récursif. On ne
	 * gere pas pour l'instant le cas d'erreur ou n est inferieur a 1.
	 * 
	...
	public static long factRec(int n) throws IllegalArgumentException {
		if (n < 0) {
			throw new IllegalArgumentException("l'argument n doit etre positif : " + n);
		}
		if (n == 1) {
			return 1;
		}
	...
	/**
	 * Calcule la factorielle d'un nombre entier avec un algorithme itératif. On ne
	 * gere pas pour l'instant le cas d'erreur ou n est inferieur a 1.
	 * 
	 * @param n le nombre dont on retourne la factorielle
	 * @return la factorielle de l'entier n
	 */
	public static long factIt(int n) throws IllegalArgumentException {
		long res = 1;
		if (n < 0) {
			throw new IllegalArgumentException("l'argument n doit etre positif : " + n);
		}
	...
	public static long comb(int n, int p) {
		long t;
		try {
			t = factRec(n) / (factRec(p) * factRec(n - p));
			return t;
	...
	public static long puissance(int n, int m) throws IllegalArgumentException{
		if(m<0) {
			throw new IllegalArgumentException("l'exposant ne doit pas etre un nombre negatif : "+m);
		}
		if (m == 0)
			return 1;
